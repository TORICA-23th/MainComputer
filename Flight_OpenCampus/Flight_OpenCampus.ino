...
void TWE_downlink() {
  ...
  static uint32_t TWE_last_send_time = millis() - 300;

  if (millis() - TWE_last_send_time >= 100){
    if(data_under_urm_altitude_m != 10.0){
      sprintf(TWE_BUF, "%+06.2f", data_under_urm_altitude_m);
      SerialTWE.println(TWE_BUF);
